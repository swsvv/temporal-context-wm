...
from pldm.optimizers.schedulers import LRSchedule, Scheduler
...
class TrainConfig(ConfigBase):
    ...
    def __post_init__(self):
        ...
        self.run_name = f"{self.run_group}-{current_time}-{self.seed}"

        if self.output_dir is not None:
            self.output_dir = f"{self.output_dir}_{current_time}"
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.epoch, self.config.epochs + 1), desc="Epoch"):
            ...
            for step, batch in (
                ...
                forward_result = self.model.forward_posterior(
                    s,
                    a,
                    **optional_fields,
                )
