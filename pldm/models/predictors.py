...
# NOTE: ADDED FOR ENSEMBLE
# NOTE: NOT IMPLEMENTED IN ORIGINAL REPO
class EnsemblePredictor(torch.nn.Module):
    def __init__(self, ensemble_size, builder_fn, **kwargs):
        super().__init__()
        self.ensemble_size = ensemble_size
        self.models = nn.ModuleList(
            [builder_fn(**kwargs) for _ in range(ensemble_size)]
        )

    def forward_multiple(self, *args, **kwargs):
        outputs = [model.forward_multiple(*args, **kwargs) for model in self.models]

        # Stack PredictorOutput tensor to K dimension (dim=0)
        # Modify predictions shape [T, BS, Dim] -> [K, T, BS, Dim] (if there is no K dimension)
        stacked_predictions = torch.stack([out.predictions for out in outputs], dim=0)

        stacked_obs = (
            torch.stack([out.obs_component for out in outputs], dim=0)
            if outputs[0].obs_component is not None
            else None
        )
        stacked_propio = (
            torch.stack([out.propio_component for out in outputs], dim=0)
            if outputs[0].propio_component is not None
            else None
        )

        return PredictorOutput(
            predictions=stacked_predictions,
            obs_component=stacked_obs,
            propio_component=stacked_propio,
            prior_mus=None,
            prior_vars=None,
            prior_logits=None,
            priors=None,
