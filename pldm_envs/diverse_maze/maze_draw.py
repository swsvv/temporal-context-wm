...
class D4RLMaze2DDrawer(object):
    ...
    def render_state(self, obs: Any) -> np.ndarray:
        ...
        # NOTE: Update target marker before rendering (FIXED for put the red point insde the maze) - HIDE
        if hasattr(self.env, "sim"):
            site_id = self.env.sim.model.site_name2id("target_site")
            self.env.sim.model.site_rgba[site_id] = [0, 0, 0, 0]
